//! Dynamixel XC330M model definitions.

use crate::model;

model![XC330M => {
    ModelNumber: 0, 2, None,
    ModelInformation: 2, 4, None,
    FirmwareVersion: 6, 1, None,
    Id: 7, 1, None,
    BaudRate: 8, 1, None,
    ReturnDelayTime: 9, 1, Some(UnitScale::new(Unit::Second, 0.000002f32)),
    DriveMode: 10, 1, None,
    OperatingMode: 11, 1, None,
    SecondaryId: 12, 1, None,
    HomingOffset: 20, 4, Some(UnitScale::new(Unit::Pulse, 1f32)),
    MovingThreshold: 24, 4, Some(UnitScale::new(Unit::RevPerMinute, 0.229f32)),
    TemperatureLimit: 31, 1, Some(UnitScale::new(Unit::DegreesCelsius, 1f32)),
    MaxVoltageLimit: 32, 2, Some(UnitScale::new(Unit::Volt, 0.1f32)),
    MinVoltageLimit: 34, 2, Some(UnitScale::new(Unit::Volt, 0.1f32)),
    PwmLimit: 36, 2, Some(UnitScale::new(Unit::Percent, 0.113f32)),
    CurrentLimit: 38, 2, Some(UnitScale::new(Unit::Ampere, 0.001f32)),
    VelocityLimit: 44, 4, Some(UnitScale::new(Unit::RevPerMinute, 0.229f32)),
    MaxPositionLimit: 48, 4, Some(UnitScale::new(Unit::Pulse, 1f32)),
    MinPositionLimit: 52, 4, Some(UnitScale::new(Unit::Pulse, 1f32)),
    StartupConfiguration: 60, 1, None,
    Shutdown: 63, 1, None,
    TorqueEnable: 64, 1, None,
    Led: 65, 1, None,
    StatusReturnLevel: 68, 1, None,
    RegisteredInstruction: 69, 1, None,
    HardwareErrorStatus: 70, 1, None,
    VelocityIGain: 76, 2, None,
    VelocityPGain: 78, 2, None,
    PositionPGain: 84, 2, None,
    BusWatchdog: 98, 1, Some(UnitScale::new(Unit::Second, 0.02f32)),
    GoalPwm: 100, 2, Some(UnitScale::new(Unit::Percent, 0.113f32)),
    GoalCurrent: 102, 2, Some(UnitScale::new(Unit::Ampere, 0.001f32)),
    GoalVelocity: 104, 4, Some(UnitScale::new(Unit::RevPerMinute, 0.229f32)),
    ProfileAcceleration: 108, 4, Some(UnitScale::new(Unit::RevPerMinuteSquared, 214.577f32)),
    ProfileVelocity: 112, 4, Some(UnitScale::new(Unit::RevPerMinute, 0.229f32)),
    GoalPosition: 116, 4, Some(UnitScale::new(Unit::Pulse, 1f32)),
    Moving: 122, 1, None,
    MovingStatus: 123, 1, None,
    PresentPwm: 124, 2, Some(UnitScale::new(Unit::Percent, 0.113f32)),
    PresentCurrent: 126, 2, Some(UnitScale::new(Unit::Ampere, 0.001f32)),
    PresentVelocity: 128, 4, Some(UnitScale::new(Unit::RevPerMinute, 0.229f32)),
    PresentPosition: 132, 4, Some(UnitScale::new(Unit::Pulse, 1f32)),
    VelocityTrajectory: 136, 4, Some(UnitScale::new(Unit::RevPerMinute, 0.229f32)),
    PositionTrajectory: 140, 4, Some(UnitScale::new(Unit::Pulse, 1f32)),
    PresentInputVoltage: 144, 2, Some(UnitScale::new(Unit::Volt, 0.1f32)),
    PresentTemperature: 146, 1, Some(UnitScale::new(Unit::DegreesCelsius, 1f32)),
    BackupReady: 147, 1, None,
    PositionDGain: 80, 2, None,
    PositionIGain: 82, 2, None,
    ProtocolType: 13, 1, None,
    Feedforward2NdGain: 88, 2, None,
    Feedforward1StGain: 90, 2, None,
    RealtimeTick: 120, 2, Some(UnitScale::new(Unit::Second, 0.001f32)),
    PwmSlope: 62, 1, Some(UnitScale::new(Unit::VoltPerSecond, 1.977f32)),
}];
